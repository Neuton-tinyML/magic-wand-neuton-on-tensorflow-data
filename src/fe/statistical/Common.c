...
void dsp_init_lib(const uint16_t *config)
{
	...
	if (dspIsInited == 0)
	{
		for (uint16_t column = 0; column < NEUTON_MODEL_USED_ORIGINAL_INPUTS_COUNT; ++column)
		{
			...
			dspAxisInfo[column].fhtInstance.fhtWindowSize = fftWindowSize;
			dspAxisInfo[column].fhtInstance.logN = fftLogN;
			dspAxisInfo[column].fhtInstance.reversedBitIndexArray = fftReversedBitIndexArray;
			dspAxisInfo[column].fhtInstance.sinTable = fftSinTable;
			dspAxisInfo[column].fhtInstance.tgTable = fftTgTable;
			dspAxisInfo[column].fhtInstance.windowArray = fftWindow;
#endif
...
{		
	...
	*p_calc_bit_mask_t = dspAxisInfo[p_ColIdx].calcBitMask;
}
...
{
	*instanceToGet = &dspAxisInfo[axisIdx].fhtInstance;
}

#else // else not PYTHON_LIB

// Functions used in python library during training
// Python library allocated it's own structures
